...
public class Raycast {
    ...
    private bool Target() {
        ...
        Vector3 origemRaio = player.position;
        Vector3 direcaoRaio = player.direction;
        float distanciaMaxima = 5.0f;

        // Inicializa o algoritmo DDA
        int mapaX = (int)Math.Floor(origemRaio.X);
        int mapaY = (int)Math.Floor(origemRaio.Y);
        int mapaZ = (int)Math.Floor(origemRaio.Z);

        float deltaDistX = Math.Abs(1 / direcaoRaio.X);
        float deltaDistY = Math.Abs(1 / direcaoRaio.Y);
        float deltaDistZ = Math.Abs(1 / direcaoRaio.Z);

        float ladoDistX, ladoDistY, ladoDistZ;
        int passoX, passoY, passoZ;

        // Determina a direção do passo e distâncias laterais iniciais
        if (direcaoRaio.X < 0) {
            passoX = -1;
            ladoDistX = (origemRaio.X - mapaX) * deltaDistX;
        } else {
            passoX = 1;
            ladoDistX = (mapaX + 1.0f - origemRaio.X) * deltaDistX;
        }

        if (direcaoRaio.Y < 0) {
            passoY = -1;
            ladoDistY = (origemRaio.Y - mapaY) * deltaDistY;
        } else {
            passoY = 1;
            ladoDistY = (mapaY + 1.0f - origemRaio.Y) * deltaDistY;
        }

        if (direcaoRaio.Z < 0) {
            passoZ = -1;
            ladoDistZ = (origemRaio.Z - mapaZ) * deltaDistZ;
        } else {
            passoZ = 1;
            ladoDistZ = (mapaZ + 1.0f - origemRaio.Z) * deltaDistZ;
        }

        // Algoritmo DDA
        int lado = 0; // Qual lado foi atingido (0=x, 1=y, 2=z)
        float distancia = 0;

        while (distancia < distanciaMaxima) {
            ...
            distancia = Math.Min(ladoDistX, Math.Min(ladoDistY, ladoDistZ));

            // Verifica se este bloco é sólido
            if (level.IsSolidTile(mapaX, mapaY, mapaZ)) {
                hitResult.x = mapaX;
                hitResult.y = mapaY;
                hitResult.z = mapaZ;
                ...
                if (lado == 0) {
                    hitResult.f = (passoX > 0) ? 0 : 1; // Oeste ou Leste
                } else if (lado == 1) {
                    hitResult.f = (passoY > 0) ? 2 : 3; // Baixo ou Topo
                } else {
                    hitResult.f = (passoZ > 0) ? 4 : 5; // Norte ou Sul
                }
